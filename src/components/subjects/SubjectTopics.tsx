...
import type { SubjectScope } from "../../lib/subjectScope";

export function SubjectTopics({ context }: { context: SubjectScope }) {
  const subject = context.subject;
  const { attempts } = useAttempts();
  const activeExamBoard = context.examBoard;
  const activeQualification = context.qualification;
  ...
  const userAttempts = attempts.filter(
    (a) =>
      a.examBoard === activeExamBoard &&
      a.qualification === activeQualification &&
      a.subject === subject,
  );
